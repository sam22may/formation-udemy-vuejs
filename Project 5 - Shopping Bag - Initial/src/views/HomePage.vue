	...
	<div class="home">
		<div class="products">
			<div v-for="(product, index) in products" :key="index" class="product" :class="{ inBag: isInBag(product) }">
				...
				<button v-if="!isInBag(product)" @click="($event) => addToBag(product)">Add to bag</button>
				<button v-else class="remove" @click="($event) => removeFromBag(product)">Remove from bag</button>
			</div>
		</div>
	</div>
</template>
